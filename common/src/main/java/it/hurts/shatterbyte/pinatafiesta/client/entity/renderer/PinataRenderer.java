...
public class PinataRenderer extends LivingEntityRenderer<PinataEntity, PinataRenderState, PinataModel> {
    ...
    public void extractRenderState(PinataEntity entity, PinataRenderState state, float partialTick) {
        ...
        state.spawnAlpha = Mth.clamp((entity.tickCount + partialTick) / 10F, 0F, 1F);
    }
    ...
    @Override
    protected RenderType getRenderType(PinataRenderState state, boolean bodyVisible, boolean translucent, boolean glowing) {
        if (state.spawnAlpha < 1.0F) {
    ...
    @Override
    protected int getModelTint(PinataRenderState state) {
        int alpha = (int) (Mth.clamp(state.spawnAlpha, 0F, 1F) * 255F);

        return alpha << 24 | 0xFFFFFF;
    }

    @Override
    protected float getShadowRadius(PinataRenderState state) {
        return super.getShadowRadius(state) * state.spawnAlpha;
    }
